...
jaccard <- read_csv(jaccard_fp, col_names=FALSE)
colnames(jaccard) <- c("seqnames","start","end","width","strand","sampleID","hgncSymbol","type","pValue","padjust","psiValue","deltaPsi",
...
########################################################################
##############-----Get Significant MIGs with Outliers-----##############
########################################################################
...
##############-----Psi3-----##############
results_filtered_psi3 <- all_uncompiled %>% filter(!is.na(hgncSymbol)) %>% filter(padjust < 0.05) %>% filter(abs(deltaPsi) >= 0.3) %>% 
                             filter(type == "psi3")
results_filtered_psi3 <- results_filtered_psi3 %>% select("sampleID", "hgncSymbol") %>% filter(hgncSymbol %in% MIG_table_select$gene_symbol) %>% unique()
colnames(results_filtered_psi3) <- c("sampleID", "gene_symbol")
results_filtered_MIGs_psi3 <- results_filtered_psi3 %>% group_by(sampleID) %>% tally() %>% arrange(desc(n))
colnames(results_filtered_MIGs_psi3) <- c("sampleID", "no_MIGs_with_psi3_juncs")

all_count <- left_join(all_count, results_filtered_MIGs_psi3)

all_count <- all_count %>% filter(! sampleID %in% missing_metadata$sampleID)
all_count[is.na(all_count)] <- 0

##############-----Psi3-----##############
results_filtered_psi5 <- all_uncompiled %>% filter(!is.na(hgncSymbol)) %>% filter(padjust < 0.05) %>% filter(abs(deltaPsi) >= 0.3) %>% 
                             filter(type == "psi5")
results_filtered_psi5 <- results_filtered_psi5 %>% select("sampleID", "hgncSymbol") %>% filter(hgncSymbol %in% MIG_table_select$gene_symbol) %>% unique()
colnames(results_filtered_psi5) <- c("sampleID", "gene_symbol")
results_filtered_MIGs_psi5 <- results_filtered_psi5 %>% group_by(sampleID) %>% tally() %>% arrange(desc(n))
colnames(results_filtered_MIGs_psi5) <- c("sampleID", "no_MIGs_with_psi5_juncs")

all_count <- left_join(all_count, results_filtered_MIGs_psi5)

all_count <- all_count %>% filter(! sampleID %in% missing_metadata$sampleID)
all_count[is.na(all_count)] <- 0

##############-----Jaccard-----##############
results_filtered_jaccard <- jaccard %>% filter(!is.na(hgncSymbol)) %>% filter(padjust < 0.05) %>% filter(abs(deltaPsi) >= 0.3) %>% 
                             filter(type == "jaccard")
results_filtered_jaccard <- results_filtered_jaccard %>% select("sampleID", "hgncSymbol") %>% filter(hgncSymbol %in% MIG_table_select$gene_symbol) %>% unique()
colnames(results_filtered_jaccard) <- c("sampleID", "gene_symbol")
results_filtered_MIGs_jaccard <- results_filtered_jaccard %>% group_by(sampleID) %>% tally() %>% arrange(desc(n))
colnames(results_filtered_MIGs_jaccard) <- c("sampleID", "no_MIGs_with_jaccard_juncs")

all_count <- left_join(all_count, results_filtered_MIGs_jaccard)
