...
counts_fp <- "/home/maurertm/smontgom/shared/UDN/Analysis/Transcriptome_Wide_Splicing_Analysis/Paper/run_results_muscular_dystrophy/output/dataframes/counts.csv"
counts <- read_csv(counts_fp)
output_file <- "/home/maurertm/smontgom/shared/UDN/Analysis/Transcriptome_Wide_Splicing_Analysis/Paper/run_results_muscular_dystrophy/output/dataframes/outliers.csv"
...
Psi3_Psi5_Junctions <- get_two_sd_status(counts, "Psi3_Psi5_Junctions")
Psi3_Psi5_Junctions_01 <- get_two_sd_status(counts, "Psi3_Psi5_Junctions_01")
Psi3_Psi5_Junctions_001 <- get_two_sd_status(counts, "Psi3_Psi5_Junctions_001")
Psi3_Psi5_Junctions_1e_6 <- get_two_sd_status(counts, "Psi3_Psi5_Junctions_1e_6")
...
Psi3_Psi5_Genes <- get_two_sd_status(counts, "Psi3_Psi5_Genes")
Psi3_Psi5_Genes_01 <- get_two_sd_status(counts, "Psi3_Psi5_Genes_01")
Psi3_Psi5_Genes_001 <- get_two_sd_status(counts, "Psi3_Psi5_Genes_001")
Psi3_Psi5_Genes_1e_6 <- get_two_sd_status(counts, "Psi3_Psi5_Genes_1e_6")
...
joined_filtered <- left_join(All_Genes, Psi3_Genes) %>% left_join(Psi5_Genes) %>% left_join(Theta_Genes) %>% left_join(Jaccard_Genes) %>% 
                ...
                left_join(Psi3_Psi5_Genes) %>% left_join(Psi3_Psi5_Genes_01) %>% left_join(Psi3_Psi5_Genes_001) %>% left_join(Psi3_Psi5_Genes_1e_6) %>%
                left_join(Psi3_Psi5_Junctions) %>% left_join(Psi3_Psi5_Junctions_01) %>% left_join(Psi3_Psi5_Junctions_001) %>% left_join(Psi3_Psi5_Junctions_1e_6)
