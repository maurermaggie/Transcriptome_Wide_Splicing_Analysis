...
genes <- read_rds("/home/maurertm/smontgom/shared/UDN/Analysis/FRASER/pipelines/FRASER_snakemake/output/Subset_25_filtered/FRASER_filtered.rds")
...
filtered_gene_list <- fds_annotated@rowRanges %>% as.data.frame %>% filter(!is.na(hgnc_symbol)) %>% pull(hgnc_symbol)
...
outlier_genes <- all_uncompiled %>% filter(padjust <= 0.05) %>% filter(abs(zScore) >= 2) %>% filter(abs(deltaPsi) >= 0.3) %>% select(hgncSymbol) %>% filter(!is.na(hgncSymbol)) %>% unique
...
ggsave(filename="/home/maurertm/smontgom/shared/UDN/Analysis/Transcriptome_Wide_Splicing_Analysis/Paper/Figure_1/Figure1c.pdf", plot=forest_plot_odds_ratio,  limitsize = FALSE, units = "in")
