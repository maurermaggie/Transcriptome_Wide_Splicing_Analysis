...
metadata <- joined_filtered %>% select(sampleID, affected_status)
all_uncompiled <- left_join(all_uncompiled, metadata)
cases <- all_uncompiled %>% filter(affected_status == "Case")
controls <- all_uncompiled %>% filter(affected_status == "Control")

outlier_genes_05_case <- cases %>% filter(padjust <= 0.05) %>% filter(abs(zScore) >= 2) %>% filter(abs(deltaPsi) >= 0.3) %>% filter(! sampleID %in% low_RIN$sampleID) %>% select(hgncSymbol) %>% filter(!is.na(hgncSymbol)) %>% unique
outlier_genes_05_case_fp <- fisher_function(outlier_genes_05_case, filtered_genes)


outlier_genes_05_control <- controls %>% filter(padjust <= 0.05) %>% filter(abs(zScore) >= 2) %>% filter(abs(deltaPsi) >= 0.3) %>% filter(! sampleID %in% low_RIN$sampleID) %>% select(hgncSymbol) %>% filter(!is.na(hgncSymbol)) %>% unique
outlier_genes_05_control_fp <- fisher_function(outlier_genes_05_control, filtered_genes)
