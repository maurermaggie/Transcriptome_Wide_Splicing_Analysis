...
forest_plot_function <- function(outlier_genes, filtered_gene_list) {
            ...
            pvalue <- c(HAP_tidy$p.value, OL_tidy$p.value, DD_tidy$p.value, OMIM_tidy$p.value, Non_Essential_tidy$p.value, Recessive_tidy$p.value, Dominant_tidy$p.value, LoF_tidy$p.value)
            mean <- c((HAP_tidy$estimate), (OL_tidy$estimate), (DD_tidy$estimate), (OMIM_tidy$estimate), (Non_Essential_tidy$estimate), (Recessive_tidy$estimate), (Dominant_tidy$estimate), (LoF_tidy$estimate))
            conf.low <- c((HAP_tidy$conf.low), (OL_tidy$conf.low), (DD_tidy$conf.low), (OMIM_tidy$conf.low), (Non_Essential_tidy$estimate), (Recessive_tidy$conf.low), (Dominant_tidy$conf.low), (LoF_tidy$conf.low))
            conf.high <- c((HAP_tidy$conf.high), (OL_tidy$conf.high), (DD_tidy$conf.high), (OMIM_tidy$conf.high), (Non_Essential_tidy$estimate), (Recessive_tidy$conf.high), (Dominant_tidy$conf.high), (LoF_tidy$conf.high))

            LoF_Tag <- paste("Homozygous Loss of", "\n", "Function Tolerant")
            CRISPR_Non_Essential_Tag <- paste("CRISPR", "\n", "Non Essential")
            OL <- paste("Olfactory", "\n", "receptor")
            DD <- paste("Developmental", "\n", "Delay")
            values <- c("Haploinsufficient", OL, DD, "OMIM", CRISPR_Non_Essential_Tag, "Recessive", "Dominant", LoF_Tag)
            ...
            forest_plot_odds_ratio <- ggplot(data=forest_plot_data, aes(colour = values, x=values, y=mean, ymin=conf.low, ymax=conf.high)) +
                    ...
                    scale_colour_manual(values = pal[c(3,2,10,4,5,1, 8,9)]) +
                    ylab("Odds Ratio") +
...
p_values <- c(1.305111e-01, 3.135457e-03, 1.510683e-12, 6.437797e-39, 0.000000e+00, 7.153273e-01, 5.673406e-02, 2.350456e-02, 4.226780e-03)
