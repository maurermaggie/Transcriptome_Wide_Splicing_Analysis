...
#0.01
psi3_count_01 <- filter_genes(all_uncompiled, "psi3", "Psi3_01", 0.01)
psi5_count_01 <- filter_genes(all_uncompiled, "psi5", "Psi5_01", 0.01)
theta_count_01 <- filter_genes(all_uncompiled, "theta", "Theta_01", 0.01)
...
joined <- left_join(psi3_count, psi5_count) %>% left_join(theta_count) %>% left_join(psi3_count_1e_6) %>% left_join(psi5_count_1e_6) %>% left_join(theta_count_1e_6) %>% 
                  left_join(psi3_count_001) %>% left_join(psi5_count_001) %>% left_join(theta_count_001) %>% left_join(psi3_count_01) %>% left_join(psi5_count_01) %>% left_join(theta_count_01)
...
significance_plot_psi3 <- ggplot(joined, aes(x = reorder(sampleID, Psi3), y = Psi3)) +
  geom_point(shape=21, alpha=.6, size=5, fill="red", aes(y=Psi3, x = reorder(sampleID, Psi3), colour = "<= 0.05")) +
  geom_point(shape=21, alpha=.6, size=5, fill="orange", aes(y=Psi3_1e_6, x = reorder(sampleID, Psi3_1e_6), colour = "<= 0.001")) +
  geom_point(shape=21, alpha=.6, size=5, fill="green", aes(y=Psi3_001, x = reorder(sampleID, Psi3_001), colour = "<= 1e-6")) +
  geom_point(shape=21, alpha=.6, size=5, fill="blue", aes(y=Psi3_01, x = reorder(sampleID, Psi3_01), colour = "<= 0.01")) +
  xlab(x_lab) +
  ylab(y_lab) +
  ...
  labs(color="Pvalue Filter")

significance_plot_psi3

ggsave(filename="/home/maurertm/smontgom/shared/UDN/Analysis/Transcriptome_Wide_Splicing_Analysis/Paper/Supplemental/Significance_geom_points/plots/significance_psi3.pdf", plot=significance_plot_psi3,  limitsize = FALSE, units = "in")
...
significance_plot_psi5 <- ggplot(joined, aes(x = reorder(sampleID, Psi5), y = Psi5)) +
  geom_point(shape=21, alpha=.6, size=5, fill="red", aes(y=Psi5, x = reorder(sampleID, Psi5), colour = "<= 0.05")) +
  geom_point(shape=21, alpha=.6, size=5, fill="orange", aes(y=Psi5_1e_6, x = reorder(sampleID, Psi5_1e_6), colour = "<= 0.001")) +
  geom_point(shape=21, alpha=.6, size=5, fill="green", aes(y=Psi5_001, x = reorder(sampleID, Psi5_001), colour = "<= 1e-6")) +
  geom_point(shape=21, alpha=.6, size=5, fill="blue", aes(y=Psi3_01, x = reorder(sampleID, Psi5_01), colour = "<= 0.01")) +
  xlab(x_lab) +
  ylab(y_lab) +
  labs(title=title_lab)  +
  #geom_point(data=g1, aes(colour= numbers)) +
...
ggsave(filename="/home/maurertm/smontgom/shared/UDN/Analysis/Transcriptome_Wide_Splicing_Analysis/Paper/Supplemental/Significance_geom_points/plots/significance_psi5.pdf", plot=significance_plot_psi5,  limitsize = FALSE, units = "in")
...
significance_plot_theta <- ggplot(joined, aes(x = reorder(sampleID, Theta), y = Theta)) +
  geom_point(shape=21, alpha=.6, size=5, fill="red", aes(y=Theta, x = reorder(sampleID, Theta), colour = "<= 0.05")) +
  geom_point(shape=21, alpha=.6, size=5, fill="orange", aes(y=Theta_1e_6, x = reorder(sampleID, Theta_1e_6), colour = "<= 0.001")) +
  geom_point(shape=21, alpha=.6, size=5, fill="green", aes(y=Theta_001, x = reorder(sampleID, Theta_001), colour = "<= 1e-6")) +
  geom_point(shape=21, alpha=.6, size=5, fill="blue", aes(y=Theta_01, x = reorder(sampleID, Theta_01), colour = "<= 0.01")) +
  xlab(x_lab) +
  ylab(y_lab) +
  labs(title=title_lab)  +
  #geom_point(data=g1, aes(colour= numbers)) +
...
ggsave(filename="/home/maurertm/smontgom/shared/UDN/Analysis/Transcriptome_Wide_Splicing_Analysis/Paper/Supplemental/Significance_geom_points/plots/significance_theta.pdf", plot=significance_plot_theta,  limitsize = FALSE, units = "in")
