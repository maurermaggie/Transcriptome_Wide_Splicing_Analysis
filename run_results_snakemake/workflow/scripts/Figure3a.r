...
MIGs_RNU4ATAC <- ggplot(joined, aes(x = reorder(numbers, no_theta_juncs_in_MIGs), y = no_theta_juncs_in_MIGs)) +
  ...
  geom_text_repel(data=(joined %>% filter(type %in% top_10)), size=8, aes(x = reorder(type, no_theta_juncs_in_MIGs), y = no_theta_juncs_in_MIGs, label = type), position=position_dodge(width=0.9),
...
ggsave(filename=geom_point_RNU4ATAC, plot=MIGs_RNU4ATAC,  limitsize = FALSE, units = "in")
...
RD268_no <- joined %>% filter(sampleID == "RD268") %>% pull(no_theta_juncs_in_MIGs) %>% length
...
GSS225379_no <- joined %>% filter(sampleID == "GSS225379") %>% pull(no_theta_juncs_in_MIGs) %>% length
...
UDN550488.Aligned.sortedByCoord.out.bam_no <- joined %>% filter(sampleID == "UDN550488.Aligned.sortedByCoord.out.bam") %>% pull(no_theta_juncs_in_MIGs) %>% length
...
UDN238929.Aligned.sortedByCoord.out.bam_no <- joined %>% filter(sampleID == "UDN238929.Aligned.sortedByCoord.out.bam") %>% pull(no_theta_juncs_in_MIGs) %>% length
...
RD380_no <- joined %>% filter(sampleID == "RD380") %>% pull(no_theta_juncs_in_MIGs) %>% length
