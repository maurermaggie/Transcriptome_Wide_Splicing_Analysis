...
non_RNU6ATAC <- joined %>% arrange(desc(no_theta_juncs_in_MIGs)) %>% select(sampleID, no_theta_juncs_in_MIGs, numbers) %>% filter(! sampleID %in% c("RD380"))
...
MIGs_RNU6ATAC <- ggplot(joined, aes(x = reorder(numbers, no_theta_juncs_in_MIGs), y = no_theta_juncs_in_MIGs)) +
...
ggsave(filename=geom_point_RNU6ATAC, plot=MIGs_RNU6ATAC,  limitsize = FALSE, units = "in")
